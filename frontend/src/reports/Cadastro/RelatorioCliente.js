...
export const RelatorioCliente = (data) => {
  const docDefinition = {
    pageSize: 'A4',
...

      {
        table: {
          ...
          body: [
            [
              {
                text: 'Relatórios de Cadastros - Clientes',
                fontSize: 16,
                bold: true,
...

      {
        table: {
          ...
          body: [
            ...
            ...(data || []).map((item) => [
              { text: item.id_cliente, fontSize: 10 }, // Adjust the font size if needed
              { text: item.nome, fontSize: 10 }, // Adjust the font size if needed
              { text: item.tipo_cliente, fontSize: 10 }, // Adjust the font size if needed
