...
const ListarOrcamentoProduto = () => {
  ...
  const { listAllBudget, deleteBudget, listBudgetToPrint, listHistoryBudget, loading, updateStatusBudget } = useBudget();
  ...
  const ActionsDropdown = ({ client }) => {
    ...
    const navItems = [
      ...
      },
      {
        ...
        icon: 'lnr-cart',
        color: 'success',
        onClick: () => navigate(`/venda/produto/gerar-venda=true/${client.id}`),
      }
  ...
  const clients = budgets?.map((budget) => {
    return {
      ...
      status: <div className="ms-auto badge" style={{ backgroundColor: budget.situacao_venda.cor }}>{budget.situacao_venda.descricao}</div>,
