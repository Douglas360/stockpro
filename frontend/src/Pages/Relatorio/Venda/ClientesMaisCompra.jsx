...
import { RelatorioClienteMaisCompra } from '../../../reports/Venda/RelatorioClienteMaisCompram'

export const ClientesMaisCompra = () => {
  const idEmpresa = sessionStorage?.getItem('user') || localStorage?.getItem('user')
  const id = JSON.parse(idEmpresa).id_empresa
  const { getReportCustomerBuy, loading } = useReport()
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    const { data_inicial, data_final, cliente, situacao, orderBy } = e.target
    const data = {
      "report": {
        ...
        orderBy: Number(orderBy.value),
        id_empresa: id
      }
    }
    const response = await getReportCustomerBuy(data)
    if (response) {
      ...
      RelatorioClienteMaisCompra(response)
  ...
  return (
    <>
      <PageTitle
        heading={'Relatório de Clientes que mais compram'}
        subheading="Relatório de Clientes que mais compram"
